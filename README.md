...
```
src/
├── content.config.ts          # Content Collectionsのスキーマ定義(このメディア特有: tournament/players/sources/relatedServices)
...
任意フィールド: `tournament` / `players` / `sources` / `affiliateProducts` / `relatedServices`
